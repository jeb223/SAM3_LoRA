...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False):
        ...
        self.amp_enabled = self.device.type == "cuda" and mixed_precision in {"fp16", "bf16"}
        self.amp_dtype = (
    ...
    def train(self):
        ...
        if self.multi_gpu:
            ...
            print_rank0(
                f"Effective optimizer batch size with accumulation: "
                f"{self.config['training']['batch_size']} x {self.world_size} x {self.grad_accum_steps} "
                f"= {self.config['training']['batch_size'] * self.world_size * self.grad_accum_steps}"
            )
        else:
            print_rank0(
                f"Effective optimizer batch size with accumulation: "
                f"{self.config['training']['batch_size']} x {self.grad_accum_steps} "
                f"= {self.config['training']['batch_size'] * self.grad_accum_steps}"
            )
        ...
        for epoch in range(epochs):
            ...
            for step, batch_dict in enumerate(pbar):
                ...
                should_step = ((step + 1) % self.grad_accum_steps == 0) or ((step + 1) == num_train_batches)
                sync_context = self.model.no_sync() if self.multi_gpu and not should_step else contextlib.nullcontext()
                ...
                if should_step:
                    max_grad_norm = float(self.config["training"].get("max_grad_norm", 0.0))
                    if max_grad_norm > 0:
                        if self.grad_scaler.is_enabled():
