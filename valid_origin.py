...
def create_coco_gt_from_dataset_original_res(dataset, image_ids=None, debug=False):
    ...
    for coco_image_id, dataset_idx in resolved_items:
        image_info = dataset.images[int(coco_image_id)]
        source_annotations = dataset.img_to_anns.get(coco_image_id, [])
        orig_w = int(image_info["width"])
        orig_h = int(image_info["height"])
...
def validate(config_path, weights_path, val_data_dir, num_samples=None,
    ...
    # Load config before model construction so optional model modules
    # such as SRF-Lite are present before adaptation weights are loaded.
    config = None
    if use_base_model:
        batch_size = 1
    else:
    ...
    if use_srf_lite:
        print(
            "Using SRF-Lite: "
            f"num_levels={int(srf_cfg.get('num_levels', 4))}, "
            f"bottleneck_dim={srf_cfg.get('bottleneck_dim', None)}, "
            f"interpolation={str(srf_cfg.get('interpolation_mode', 'bilinear'))}, "
            f"alpha_init={float(srf_cfg.get('alpha_init', 0.0))}"
        )
    ...
    if merge_cracks:
        print(f"\n[INFO] Total predictions after CRACK MERGING: {len(coco_predictions_bbox)}")
    else:
        print(f"\n[INFO] Total predictions after SAM3 NMS filtering: {len(coco_predictions_bbox)}")
    ...
    if len(coco_predictions_bbox) > 0:
        ...
        with open(os.devnull, 'w') as devnull:
            with contextlib.redirect_stdout(devnull):
                ...
                coco_dt_bbox = coco_gt_bbox.loadRes(str(pred_bbox_file))

        print("\nBBox AP (bbox, original resolution):")
        coco_eval_bbox = run_coco_eval(coco_gt_bbox, coco_dt_bbox, 'bbox')
        map_bbox = coco_eval_bbox.stats[0]
        map50_bbox = coco_eval_bbox.stats[1]
        map75_bbox = coco_eval_bbox.stats[2]
        ...
        print(f"APbbox@50: {map50_bbox:.4f}")
        print(f"APbbox@75: {map75_bbox:.4f}")
        if has_full_gt_segmentation:
