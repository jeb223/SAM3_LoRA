...
class MultiheadAttentionLoRA(nn.Module):
    ...
    def _expand_attn_mask(
        ...
        """Expand attention masks to [batch, heads, tgt_len, src_len]."""
        if attn_mask.dim() == 2:
        ...
        if attn_mask.shape != (batch_size, self.num_heads, tgt_len, src_len):
    ...
    def _build_additive_attn_mask(
        ...
        """
        Build a single additive attention mask.

        The manual implementation follows nn.MultiheadAttention semantics:
        boolean masks use True to mean "ignore / mask out".
        """
        additive_mask = None

        if attn_mask is not None:
            attn_mask = self._expand_attn_mask(attn_mask, batch_size, tgt_len, src_len).to(device)
            if attn_mask.dtype == torch.bool:
                additive_mask = torch.zeros_like(attn_mask, dtype=dtype)
                additive_mask.masked_fill_(attn_mask, float("-inf"))
            else:
                additive_mask = attn_mask.to(dtype=dtype)
        ...
        if key_padding_mask is not None:
            ...
            padding_bias = torch.zeros_like(key_padding_mask, dtype=dtype)
            padding_bias.masked_fill_(key_padding_mask, float("-inf"))
            additive_mask = padding_bias if additive_mask is None else additive_mask + padding_bias

        if is_causal:
            causal_mask = torch.ones((tgt_len, src_len), device=device, dtype=torch.bool).triu(1)
            causal_mask = causal_mask.unsqueeze(0).unsqueeze(0).expand(batch_size, self.num_heads, tgt_len, src_len)
            causal_bias = torch.zeros_like(causal_mask, dtype=dtype)
            causal_bias.masked_fill_(causal_mask, float("-inf"))
            additive_mask = causal_bias if additive_mask is None else additive_mask + causal_bias
    ...
    def _manual_attention(
        ...
        k: torch.Tensor,
        v: torch.Tensor,
        additive_attn_mask: Optional[torch.Tensor],
        average_attn_weights: bool,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Fallback path that explicitly materializes attention weights."""
        scale = 1.0 / math.sqrt(self.head_dim)
        attn_weights = torch.matmul(q, k.transpose(-2, -1)) * scale

        if additive_attn_mask is not None:
            attn_weights = attn_weights + additive_attn_mask
    ...
    def forward(
        ...
        additive_attn_mask = self._build_additive_attn_mask(
            attn_mask=attn_mask,
