...
class CGF1Eval(COCOeval):
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        if p.useCats:
            assert catId != -1
            all_gts = self._gts[imgId, catId]
            all_dts = self._dts[imgId, catId]
        else:
            assert catId == -1
    ...
    def accumulate(self, p=None):
        ...
        self.eval = {
            ...
            "positive_macro_F1": local_F1s / max(valid_F1_count, 1),
            "IL_recall": IL_rec,
...
def _evaluate(self):
    ...
    evalImgs = np.asarray(evalImgs, dtype=object).reshape(
...
class CGF1Evaluator:
    ...
    def evaluate(self, pred_file: str):
        ...
        for img_id in tqdm(self.eval_img_ids, disable=not self.verbose):
            ...
            all_eval_imgs.extend(selected.flatten().tolist())
    ...
    def _select_best_scoring(scorings):
        ...
        def _score(scoring_slice):
            local_scores = []
            for result in scoring_slice.flatten():
                if result is None:
                    continue
                if "local_F1s" in result:
                    local_scores.append(float(np.mean(result["local_F1s"])))
                else:
                    # Empty GT/pred is treated as a perfect local result.
